...
variable "vnets" {
  type = list(object({
    ...
    subnets = optional(list(object({
      ...
      existing_network_security_group = optional(object({
        id                  = optional(string, null)
        name                = optional(string, null)
        resource_group_name = optional(string, null)
      }), null)
  ...
  description = <<-DESC
    A list of one or more Virtual Networks to create.
    `name`: (Required) The name to give the Virtual Network.
    `address_space`: (Optional) The Virtual Network's address space in CIDR format.
    `location`: (Optional) The location in which the Virtual Network should be created. Defaults to module's default, if not provided.

    `peerings`: (Optional) A list of remote Virtual Networks to which this Virtual Network should be peered.
    ...
    `subnets`: (Optional) A list of the Subnets to create within the Virtual Network.
      `name`: (Required) The name of the Subnet.
      `address_prefixes`: (Optional) The Subnet's address space in CIDR format.
